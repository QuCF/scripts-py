...
class Reg__:
    ...
    def compute_bitstring_integer(self, ir, ic):
        integer = np.nan
        ...
        if np.isnan(integer):
            ...
            bitstring = mix.find_bit_array_of_int(integer, self.nq_)
...
class Circuit__:
    ...
    def get_anc_integers_from_section_index(self, section_id, i_anc = 0, anc_integers=None, anc_bitstrings = None):
        if i_anc == 0:
            anc_integers=[]
            anc_bitstrings = []

        if i_anc == (self.ancilla_regs_.N_regs_-1):
        ...
        bs = mix.find_bit_array_of_int(integer_anc, anc.nq_)

        anc_integers.append(integer_anc)
        anc_bitstrings.append(bs)

        i_anc += 1
        if i_anc < self.ancilla_regs_.N_regs_:
            anc_integers, anc_bitstrings = self.get_anc_integers_from_section_index(
                section_id, i_anc, anc_integers, anc_bitstrings
            )
        return anc_integers, anc_bitstrings
    

    # decompose i_to_decompose into a bistring;
    # then, split the bistring between input registers -> bitstrings_row
    # bistring in each input register corresponds to an integer -> integers_row
    def compute_input_bitstrings(self, i_to_decompose):
        N_input_regs = self.input_regs_.N_regs_
        
        # decompose ir into bitstrings for input registers:
        bitstring = mix.find_bit_array_of_int(i_to_decompose, self.input_regs_.nq_)

        # get a bitstring for each input register;
        # bitstrings_row[i][0] is the most significant bit of the i-th register:
        bitstrings_row = [None] * N_input_regs
        id_start = 0
        for ireg in range(N_input_regs):
            nq_reg = self.input_regs_.nqs_[ireg]
            bitstrings_row[ireg] = bitstring[id_start:(id_start + nq_reg)]
            id_start += nq_reg

        # get an integer for each register:
        integers_row = [None] * N_input_regs
        for ireg in range(N_input_regs):
            integers_row[ireg] = mix.find_int_from_bit_array(
                bitstrings_row[ireg]
            )
        return bitstrings_row, integers_row


    # assume that each ancilla register corresponds to another input register:
    def compute_ancilla_bitstrings(self, irs, ics):
        N_anc_regs = self.ancilla_regs_.N_regs_
        bitstrings = [None] * N_anc_regs
        integers   = [None] * N_anc_regs
        for i_anc_reg in range(N_anc_regs):
            bitstrings[i_anc_reg], integers[i_anc_reg] = \
                self.ancilla_regs_.get_reg(i_anc_reg).compute_bitstring_integer(
                    irs[i_anc_reg], 
                    ics[i_anc_reg]
                )
        return bitstrings, integers
...
# -------------------------------------------------------------------------------------------------
# --- Group class ---
# A group is several zones on a given matrix column or diagonal, 
...
class Group__:
    ...
    # row indices where various zones of the group end within a matrix;
    # the indices are sorted in the ascending order:
    irs_end_ = None

    def __init__(self, obj_group=None, angle=None):
        self.irs_start_ = []
        self.irs_end_ = []  # not including
...
class SortedGroup__:
    ...
    def compute_reg_integers_from_control_nodes(self, circ):
        ...
        for i_control_int in range(len(control_integers)):
            _, array_integers_input_regs[i_control_int,:] = \
                circ.compute_input_bitstrings(
...
def create_grid_of_gates(circ, T): 
    # initialize grid where gates will be arranged:
    grid_R = circ.init_grid_of_gates()
    N = T.shape[0]  
    for ir in range(N):  
        _, integers_row = circ.compute_input_bitstrings(ir)
        for ic in range(N):
            if np.isnan(T[ir, ic]):
                continue

            _, integers_columns = circ.compute_input_bitstrings(ic)
            _, integers_anc_column = \
                circ.compute_ancilla_bitstrings(integers_row, integers_columns)
            
            if None in integers_anc_column:
                print("Error: specified circuit structure cannot encode the given matrix.")
                return None
            
            i_section = circ.get_section_index(integers_anc_column) # !!!recheck!!!
            grid_R[i_section][ir] = T[ir, ic]
    return grid_R



# groups_R[i]: groups in the ith section;
# groups_R[i-section][i-group] = Group__(angle, [rows_start], [rows_end])
def create_groups(circ, grid_R):
...
# By using groups, reconstruct the encoded matrix.
def reconstruct_A_using_groups(circ, D, groups_Rx, groups_Ry, groups_Rz):
    ids_R = []
    dict_R = {}

    id_R = "Rx"
    ids_R.append(id_R)
    dict_R[id_R] = groups_Rx

    id_R = "Ry"
    ids_R.append(id_R)
    dict_R[id_R] = groups_Ry

    id_R = "Rz"
    ids_R.append(id_R)
    dict_R[id_R] = groups_Rz
    
    N = 1 << circ.input_regs_.nq_
    A_recon = np.zeros((N, N), dtype=complex) # reconstructed matrix
    # print("Matrix size: {:d}", N)
    # print("The total number of sections in the circuit: {:d}".format(circ.N_sections_))
    for id_R in ids_R:
        # print("\n --------------------------------")
        # print("Evaluate groups for the gate {:s}".format(id_R))
        groups_R = dict_R[id_R]
        ...
        for i_section in range(circ.N_sections_):
            ...
            # print()
            # print("i-section: {:d}".format(i_section))

            anc_integers, _ = circ.get_anc_integers_from_section_index(i_section)

            # for ii in range(circ.ancilla_regs_.N_regs_):
            #     print("{:s}: {:d}".format(circ.ancilla_regs_.names_[ii], anc_integers[ii]))

            N_groups = len(one_section)
            # print("Number of groups: {:d}".format(N_groups))
            for i_group in range(N_groups): # consider all groups whithin each section;
                ...
                for i_zone in range(N_zones): # consider all zones within each group;
                    ...
                    for counter_row in range(N_rows): # consider all rows within each zone;
                        ...
                        # get ic:
                        _, integers_input_regs = circ.compute_input_bitstrings(ir)
                        ic = circ.get_column_index_from_anc_integers(anc_integers, integers_input_regs)

                        # compute R:
                        A_recon[ir, ic] = apply_gate(A_recon[ir, ic], id_R, D[ir, ic], group_angle)

        # print("The number of nonempty sections: {:d}".format(counter_sections))
    return A_recon


def apply_gate(element_value, id_R, value_D, group_angle, flag_reverse=False):
...
def find_biggest_groups(circ, groups_R):
    sorted_groups_R = [None]*circ.N_sections_
    for i_section in range(circ.N_sections_):
        one_section = groups_R[i_section]
        N_groups = len(one_section)
        if N_groups == 0:
        ...
        indices_sort_from_least_to_largest = np.argsort(group_sizes) # from smallest to largest;
        for i_group in range(N_groups): 
            i_group_prev = indices_sort_from_least_to_largest[i_group]
            sorted_groups_R[i_section][i_group] = SortedGroup__(one_section[i_group_prev].a_)
...
def extend_sorted_groups_NEW(circ, sorted_groups_R, ngrp_ext = 3):
    N_regs = circ.input_regs_.N_regs_
    for i_section in range(circ.N_sections_):
        one_section = sorted_groups_R[i_section]
        ...
        for i_group in range(N_groups):
            ...
            if N_rows > 1:
                counter_ext_group += 1
                if counter_ext_group < ngrp_ext:
                    flag_ext = True
                    matrices_bs_regs = [None] * N_regs
                    for i_reg in range(N_regs):
                        matrices_bs_regs[i_reg] = np.zeros(
                            (N_rows, circ.input_regs_.nqs_[i_reg]), dtype=int
                        )

                    for i_row in range(N_rows):
                        _, integers_row = circ.compute_input_bitstrings(one_group.irs_[i_row])
                        for i_reg in range(N_regs):
                            nq = circ.input_regs_.nqs_[i_reg]
                    ...
                    for i_reg in range(N_regs): 
                        ...
                        # reg_control_qubits[i_reg] = np.array([None] * circ.input_regs_.nqs_[i_reg])
                        reg_control_qubits[i_reg] = np.zeros(circ.input_regs_.nqs_[i_reg], dtype=int)
                        ...
                        while True:
                            ...
                            if len(np.where(matrix_bs[:, counter_qubit] == bit_inv)[0]): 
                                ...
                                counter_qubit += 1
                                if counter_qubit == circ.input_regs_.nqs_[i_reg]:
                                    break
            ...
            if flag_ext:
                one_group.construct_complement(circ)
    return sorted_groups_R



def count_groups(circ, groups_Rx, groups_Ry, groups_Rz):
    ids_R = []
    dict_R = {}
    ...
    dict_R[id_R] = groups_Ry
    print()
    for i_gate in range(len(ids_R)):
        id_R = ids_R[i_gate]
        print("--- Groups for the gate {:s} ---".format(id_R))
        groups_R = dict_R[id_R]
        counter_groups = 0
        for i_section in range(circ.N_sections_):
            one_section = groups_R[i_section]
...
def print_angles(circ, groups_R, name_gate):
    print()
    print("Group angles for the gate {:s}".format(name_gate))
...
# def extend_sorted_groups(circ, sorted_groups_R):
#     N_regs = circ.input_regs_.N_regs_
#     for i_section in range(circ.N_sections_):
#         one_section = sorted_groups_R[i_section]
#         if one_section is None:
...
#         for i_group in range(N_groups):
#             one_group = one_section[i_group]
#             # one_group.init_reg_integers(circ)
...
#                         nq = circ.input_regs_.nqs_[i_reg]
#                         matrices_bs_regs[i_reg][i_row, :] = mix.find_bit_array_of_int(integers_row[i_reg], nq)
#                         # one_group.reg_integers_[i_reg, i_row] = integers_row[i_reg]
...
#                 # guarantee that the extended group does not intersect with larger extended groups;
#                 for i_prev_groups in range(i_group):
#                     prev_group = one_section[i_prev_groups]
#                     flags_diff = [False] * N_regs  
#                     if prev_group.control_reg_bits_ is None:
#                         continue
#                     for i_reg in range(N_regs):
#                         bits_prev_group = prev_group.control_reg_bits_[i_reg]
#                         bits_curr_group = reg_control_qubits[i_reg]
#                         for i_bit in range(circ.input_regs_.nqs_[i_reg]):
#                             bit_prev = bits_prev_group[i_bit]
#                             bit_curr = bits_curr_group[i_bit]
#                             if(np.isnan(bit_curr) or np.isnan(bit_prev)):
#                                 break
#                             if(bit_prev != bit_curr):
#                                 flags_diff[i_reg] = True
#                                 break
#                     flag_ext *= all(flags_diff)
#                     if not flag_ext:
#                         break
#             else:
#                 flag_ext = False
...
# Assume that gates act on some target qubit called ae;
def construct_circuit_OH(
        path, filename,
        circ, D, 
        sorted_groups_Rx, sorted_groups_Ry, sorted_groups_Rz
):
    def write_control_integers(flag_irs, counter_gates):
        ...
        for i_gate in range(len(ids_R)):
            ...
            for i_section in range(circ.N_sections_):
                ...
                if one_section is None:
                    continue
                ancilla_integers, _ = circ.get_anc_integers_from_section_index(i_section)
                N_groups = len(one_section)
                for i_group in range(N_groups):
                    one_group = one_section[i_group]
                    if func_flag_work(one_group.flag_extended_):
                        ...
                        for i_integer in range(N_integers): 
                            ...
                            _, input_integers = circ.compute_input_bitstrings(ir)
                            one_line = "{:s} {:s} ae 1 {:19.12e} ".format(
                                keyword_gate, id_R, group_angle
                            )
                            one_line += circ.write_line_for_control_integers(input_integers,   True)
                            one_line += circ.write_line_for_control_integers(ancilla_integers, False)
                            one_line += "end_gate"
    ...
    id_R = "Rz"
    ids_R.append(id_R)
    dict_R[id_R] = sorted_groups_Rz

    id_R = "Rx"
    ids_R.append(id_R)
    dict_R[id_R] = sorted_groups_Rx

    id_R = "Ry"
    ids_R.append(id_R)
    dict_R[id_R] = sorted_groups_Ry
        ...
        for i_section in range(circ.N_sections_):
            ...
            for i_group in range(N_groups):
                ...
                if one_group.flag_extended_:
                    group_angle = one_group.a_
                    one_line = "gate {:s} ae 1 {:19.12e} ".format(id_R, group_angle)
                    one_line += one_group.write_line_for_input_control_nodes(circ)
                    one_line += circ.write_line_for_control_integers(ancilla_integers, False)
                    one_line += "end_gate"
                    file_lines.append(one_line)
                    counter_gates += 1
    ...
    file_lines.append("\n")
    ids_R.reverse()
    counter_gates = write_control_integers(False, counter_gates)

    # --- Integers to set control nodes ---
    file_lines.append("\n")
    ids_R.reverse()
    counter_gates = write_control_integers(True, counter_gates)
...
# # sorted_groups_R is a set of sorted extended groups for the gate R:
# def reconstruct_matrix_using_extended_groups(
#         circ, D, sorted_groups_Rx, sorted_groups_Ry, sorted_groups_Rz
# ):
#     ids_R = []
#     dict_R = {}
...
#     nq_tot = np.sum(np.array(circ.input_regs_.nqs_), dtype=int)
#     N_tot = 1 << nq_tot
#     A_recon = np.zeros((N_tot, N_tot), dtype=complex) # reconstructed matrix
#     for i_gate in range(len(ids_R)):
#         id_R = ids_R[i_gate]
#         groups_R = dict_R[id_R]
#         for i_section in range(circ.N_sections_):
...
#                 if one_group.flag_extended_:
#                     # --- use control nodes ---
#                     array_integers_input_regs = one_group.compute_reg_integers_from_control_nodes(circ)
#                     N_integers = len(array_integers_input_regs)
#                     for i_integer in range(N_integers):
#                         integers_input_regs = array_integers_input_regs[i_integer]
#                         ir = circ.compute_row_from_input_reg_integers(integers_input_regs) 
#                         ic = circ.get_column_index_from_anc_integers(anc_integers, integers_input_regs)
                    
#                         # compute R:
#                         if ic >= 0 and ic < N_tot:
#                             A_recon[ir, ic] = apply_gate(A_recon[ir, ic], id_R, D[ir, ic], group_angle)
#                         else:
#                             xx = 0

#                     # --- reverse the group action ---
#                     N_integers = len(one_group.irs_complement_)
#                     for i_integer in range(N_integers): 
#                         ir = one_group.irs_complement_[i_integer]
#                         _, integers_input_regs = circ.compute_input_bitstrings(ir)
#                         ic = circ.get_column_index_from_anc_integers(anc_integers, integers_input_regs)

#                         # compute R*:
#                         if ic >= 0 and ic < N_tot:
#                             A_recon[ir, ic] = apply_gate(
#                                 A_recon[ir, ic], id_R, D[ir, ic], group_angle, 
#                                 flag_reverse = True
#                             )
#                             xx = 0
#                         else:
#                             xx = 0
#                 else:
#                     # set control nodes using integers for each register:
#                     N_rows = len(one_group.irs_)
#                     for i_row in range(N_rows):
#                         ir = one_group.irs_[i_row]
#                         _, integers_input_regs = circ.compute_input_bitstrings(one_group.irs_[i_row])
#                         ic = circ.get_column_index_from_anc_integers(anc_integers, integers_input_regs)

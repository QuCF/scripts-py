...
class SystemGates__:
    ...
    def __init__(self, circ, D, groups_Ry, groups_Rc):
        self.circ_ = circ
        self.D_ = D
                ...
                for i_group in range(N_groups): # consider all groups whithin each section;
                    ...
                    for i_zone in range(len(one_group.irs_start_)): # consider all zones within each group;
                        ...
                        for counter_row in range(N_rows): # consider all rows within each zone;
                            ...
                            A_recon[ir, ic] = self.apply_gate(
                                A_recon[ir, ic], gate_type, self.D_[ir, ic], one_group.a_
                            )
        return A_recon
    

    def apply_gate(self, element_value, gate_type, value_D, group_angle):
    ...
    def split_gates_groups(self):
        ...
        for i_gate in range(len(self.gate_types_)):
            gate_type = self.gate_types_[i_gate]
            groups_R  = self.dict_gates_[gate_type]
            self.dict_gates_[gate_type] = self.split_groups(groups_R)
        return  
    
    # -> IMPORTANT: should be launch BEFORE GROUP SORTING.
    ...
    def split_groups(self, groups_R):
        new_groups_R = [None] * self.circ_.N_sections_
        for i_section in range(self.circ_.N_sections_):
            one_section = groups_R[i_section]
            if one_section is None:
                continue
            new_groups_R[i_section] = []
            ...
            for i_group in range(N_groups):
                ...
                for i_row in range(N_rows):
                    # !!! its is used (means, after sort) !!!
                    # !!! here, use irs_start and its_end !!!
                    # save id of interval where
                ...
                for counter_qubit in range(self.n_split_):
                    bit_init = matrix_bs[0, counter_qubit]
                    bit_inv  = mix.inverse_bit(bit_init)
                    id_split = len(np.where(matrix_bs[:, counter_qubit] == bit_inv)[0])
                    if id_split: 
                        # bits are not constant in the qubit counter_qubit
                        # split the group:
                        flag_split = True
                        ...
                        # split modify irs_start and irs_end, where id_split sits
                        # what if id_split is irs_start[i]
                        # what if id_split is irs_end[i]
                        # what if id_split is in-between irs_start[i] and irs_end[i]
                        # save irs_start and irs_end

                        # turn irs_start and irs_end into numpy.matrices
                        # look where id_split >= irs_start and id_split <= irs_end;

                        new_groups_R[i_section].append(l_group)
                        new_groups_R[i_section].append(r_group)
                        break
                if not flag_split:
                    new_groups_R[i_section].append(one_group)
        return new_groups_R 
...
# E.g. for Ry: A[i,j] = - D[i,j] * sin(theta/2);
        ...
        for ic in range(N):
            ...
            if(np.abs(va) > 0):
                ...
                if(np.abs(va_i) > 0):
                    ...
                    Tc_ampl[ir, ic] = 2. * np.arccos(v_amp / d_coef)
                    Tc_phase[ir, ic] = - 2. * v_phase
                # a pure real value:
                else:
                    Ty[ir,ic] = 2. * np.arccos(va_r / d_coef)
...
def create_groups(circ, grid_R_ampl, grid_R_phase=None, flag_neighbor=False):
    ...
    for i_section in range(circ.N_sections_):
        ...
        group_angle = form_angle(counter_row)
        while counter_row < N:
            ...
            while circ.compare_angles(group_angle, curr_angle): # compare with a predefined precision?
                ...
                if counter_row == N:
                    break
                curr_angle = form_angle(counter_row)
        ...
        # turn irs_start and irs_end into numpy.matrices for each matrix;          
    return groups_R

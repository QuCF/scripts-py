...
class Group__:
    ...
    def set_value(self, v):
        if np.isnan(v):
            print("Error: the element value is nan")
        if np.imag(v) == 0:
            self.v_ = np.real(v)
        else:
            self.v_ = v
    ...
    def sort_groups(self, groups_R):
        ...
        for i_section in range(self.circ_.N_sections_):
            ...
            for i_group in range(N_groups): 
                ...
                sorted_groups_R[i_section][i_group] = Group__(
                    one_section[i_group_prev].a_, 
                    one_section[i_group_prev].v_
                ) 
                ...
                if N_rows > 1:
                    ...
                    for counter_qubit in range(self.n_split_):
                        ...
                        if len(array_found): 
                            ...
                            l_group = Group__(angle=one_group.a_, v=one_group.v_)
                            r_group = Group__(angle=one_group.a_, v=one_group.v_)
...
# output: groups_R[i-section][i-group] = Group__(angle, value);
...
def create_groups_neighbor(circ, B_fixed, grid_R_ampl, grid_R_phase=None):
    # ------------------------------------------------------------------------
    def find_group(group_angle, irs_cond_group, dict_angles, one_section):
        ...
        if keyword not in dict_angles:
            ...
            for id_prev_group in range(len(dict_angles[keyword])):
                ...
                is_neighbor = True
                for counter_row in range(N_cond_rows):
                ...
                if is_neighbor:
                    ...
                    # a group can be neighbot with several previous groups;
                    # here, we consider only the first encountered neighbor group:
                    break

                # Option 2.2: the first N_min elements should be neighbor, where 
                #   N_min is the min(N_rows) in both groups.  
    ...
    for i_section in range(circ.N_sections_):
        ...
        while ir < N:
            ...
            if flag_new_group:
                ...
                oo_group = Group__(angle=group_angle, v=B_fixed[ir, ic])

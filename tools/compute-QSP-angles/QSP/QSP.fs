...
let ConvertToAnglesForParity01 (qspResult: QSPResult) =
    ...
    |> Array.concat
    |> Array.rev // This reversion is because we use left action for unitaries, 
    // while in Q# we write right-most gates in formulas on top.
...
let JacobiAngerExpansion (epsilon: float) (tau: float) =
    ...
    printfn "Chosen tau = %O" tau
    printfn "Chosen eps = %O" epsilon

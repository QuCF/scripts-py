...
def set_init_variables_Q_outgoing(Q0, x0, dd):
    ...
    vx = np.zeros((nx,nt), dtype = complex)
    vy = np.zeros((nx,nt), dtype = complex)
    Ex = np.zeros((nx,nt), dtype = complex)
    Ey = np.zeros((nx,nt), dtype = complex)
    Bz = np.zeros((nx,nt), dtype = complex)
...
def compute_Q_outgoing_DD(coef_coupling, vars_data, dd):
    ...
    def one_time_step(
        ...
        vx_new = np.zeros(nx, dtype = complex)
        vy_new = np.zeros(nx, dtype = complex)
        Ex_new = np.zeros(nx, dtype = complex)
        Ey_new = np.zeros(nx, dtype = complex)
        Bz_new = np.zeros(nx, dtype = complex)
        Q_new  = np.zeros(nx, dtype = complex)

...
# ------------------------------------------------------------------------------------
def analysis_prob(pp, qq, flag_print_opt_number = True):
    ...
    print("prob. of the good state: {:0.3e}".format(pr_good))
    print("prob. of the bad state: {:0.3e}".format(pr_bad))
    print("prob. of bad state + good state: {:0.3e}".format(pr_bad + pr_good))

    if flag_print_opt_number:
        print("the optimal number of amplifications: ", n_opt)
...
def Wavefunction_adv(obj , *args, **kwargs):
    ...
    if(type(obj) == qiskit.QuantumCircuit): # obj is a quantum circuit
        simulator = AerSimulator()
        qc_stv = obj.copy()
        qc_stv.save_statevector()
        transpiled_qc = qiskit.transpile(qc_stv, simulator)
        res_sim = simulator.run(transpiled_qc).result()
        statevec = np.asarray(res_sim.data()['statevector'])

    if(type(obj) == np.ndarray): # obj is a statevector
        statevec = np.asarray(obj)
...
def print_array(
        ...
        flag_remove_zeros=False, coef_remove_zeros=1e-16,
        flag_comma = False
    ):
    if ff[2] == "d":
        ff_line = "{:d}"
